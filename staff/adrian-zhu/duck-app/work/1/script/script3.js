...
// search patitos (proceso de patitos) .vs retreive ducks
...
function searchIDDuck (id, callback){

    var xhr = new XMLHttpRequest;
    xhr.open('GET','https://duckling-api.herokuapp.com/api/search?q=' + id);
    xhr.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 201) {
            var ducks = JSON.parse(xhr.responseText);
